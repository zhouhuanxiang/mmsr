...
class Block_Residual_Module(nn.Module):
    def __init__(self, with_bp_flow=True):
        ...
        self.sr_flow_before_split = nn.Conv2d(64, 64, 3, 1, 1, bias=True)
        self.sr_flow_after_split_1 = nn.Conv2d(64, 64, 3, 1, 1, bias=True)
        self.sr_flow_after_split_2 = nn.Conv2d(64, 64, 3, 1, 1, bias=True)
        self.sr_flow_after_split_3 = nn.Conv2d(64, 64, 3, 1, 1, bias=True)

        if self.with_bp_flow:
            self.bp_flow_before_compare = nn.Conv2d(64, 64, 3, 1, 1, bias=True)
            self.bp_flow_after_compare = arch_util.ResidualBlock_noBN(64)

        self.lrelu = nn.LeakyReLU(negative_slope=0.1, inplace=True)

    def forward(self, Ix):
        Ox_before_split = self.lrelu(self.sr_flow_before_split(Ix))
        ...
        Ox_after_split = self.lrelu(self.sr_flow_after_split_3(Ox_after_split))

        if self.with_bp_flow:
            I_x1_before_compare = self.lrelu(self.bp_flow_before_compare(Ox_before_split))
            I_x1_after_compare = I_x1_before_compare - Ix
            I_x1_after_compare = self.bp_flow_after_compare(I_x1_after_compare)
